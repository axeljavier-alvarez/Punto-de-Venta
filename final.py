...
def ventana_usuarios():
  ...
  # Crear la ventana principal
    ...
    tk.Label(ventana, text="ID de Usuario:").grid(row=0, column=0)
    entry_id = tk.Entry(ventana)
    entry_id.grid(row=0, column=1)

    tk.Label(ventana, text="Nombre de Usuario:").grid(row=1, column=0)
    ...
    tk.Label(ventana, text="Rol:").grid(row=3, column=0)
...
def agregar_producto(codigo: any = None, nombre: any = None, precio: any = None, stock: any = None, ventana: any = None):
    ...
    try:
        for producto in leer_productos():
            if  codigo.get() == producto[0]:
                messagebox.showerror("Error","El ID ingresado ya existe")
                return
...
#Panel para Registrar facturas
    ...
    btn_agregar = tk.Button(frame_stock, text="Agregar", command=lambda: agregar_item_factura( int( lista_productos.index( opc_productos.get())) , int(stock_producto.get()) ))
...
buttons = [
    ...
    ("Registro de Usuarios",ventana_usuarios,"#007BFF"),
    ("Facturación", facturacion ,"#007BFF"),
